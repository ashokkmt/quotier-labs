...
import { ListSectionDefinitions, CreateSectionDefinition, UpdateSectionDefinition, DeleteSectionDefinition, CloneSectionDefinition } from "../../../wailsjs/go/wails/SectionHandler"
...
export function SectionLibrary() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this section? Templates using it might be affected.")) return
    try {
      ...
      <div className="max-w-4xl mx-auto py-6">
        <div className="mb-6 flex items-center gap-4">
          ...
          <h2 className="text-2xl font-bold font-heading">
            {activeSection ? (activeSection.is_builtin ? "View Section" : "Edit Section") : "New Section Definition"}
          </h2>
        </div>
        <Card className="p-6">
          <SectionForm 
    ...
    <div className="max-w-5xl mx-auto py-6 space-y-6 pb-12">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage reusable section blueprints for your templates.</p>
        </div>
        ...
        <div className="space-y-8">
          {Object.entries(grouped).map(([category, items]) => (
            <div key={category} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {((items as any[]) || []).map(s => (
                  <Card key={s.id} className="p-4 flex flex-col hover:border-primary/50 transition-colors cursor-pointer" onClick={() => setEditingId(s.id)}>
                    ...
                    <div className="flex justify-end gap-2 mt-4 pt-4 border-t">
                      <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); handleClone(s.id) }}>
                        <Copy className="w-4 h-4 mr-2" /> Clone
                      </Button>
                      {!s.is_builtin && (
                        <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive hover:bg-destructive/10" onClick={(e) => { e.stopPropagation(); handleDelete(s.id) }}>
