  ...
  return (
    <div className="space-y-4">
      {layout.rows.map((row, rIndex) => (
        <div key={row.id} className="relative group border rounded-lg bg-background shadow-sm hover:border-primary/50 transition-colors">
          <div className="absolute -left-3 -top-3 hidden group-hover:flex items-center gap-1">
            ...
            <Button size="icon" variant="destructive" className="h-7 w-7 rounded-md" onClick={() => removeRow(rIndex)}>
          ...
          <div className="p-4 flex flex-wrap md:flex-nowrap gap-4">
            {row.columns.map((col, cIndex) => (
              <div 
                ...
                style={{ width: col.width === "100%" ? "100%" : col.width === "50%" ? "calc(50% - 0.5rem)" : col.width === "33%" ? "calc(33.33% - 0.66rem)" : "calc(66.66% - 0.33rem)" }}
              >
                <div className="absolute top-2 right-2 hidden group-hover/col:flex gap-1">
                  {row.columns.length === 1 && (
                    <Button size="icon" variant="secondary" className="h-6 w-6" onClick={() => splitColumn(rIndex)}>
                      <SplitSquareHorizontal className="w-3 h-3" />
                    </Button>
                  )}
                  {row.columns.length > 1 && (
                    <Button size="icon" variant="destructive" className="h-6 w-6" onClick={() => removeColumn(rIndex, cIndex)}>
                      <Trash className="w-3 h-3" />
                    </Button>
                  )}
                </div>
