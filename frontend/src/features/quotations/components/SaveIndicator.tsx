...
export function SaveIndicator({ state, lastSaved }: { state: SaveState; lastSaved: Date }) {
  ...
  if (state === 'Save failed — retrying') {
    return (
      <div className="flex items-center whitespace-nowrap text-xs text-destructive">
        <AlertCircle className="w-3 h-3 mr-1" /> Save failed — retrying
      </div>
    )
  }
  if (state === 'Unsaved changes') {
    return (
      <div className="flex items-center whitespace-nowrap text-xs text-amber-600 dark:text-amber-300">
        <Save className="w-3 h-3 mr-1" /> Unsaved changes
      </div>
    )
  }
