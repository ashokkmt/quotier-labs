...
export function CustomerCombobox({ value, onChange }: { value?: string, onChange: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      SearchCustomers(search).then(res => setCustomers(res || [])).catch(console.error)
    }, 300)
